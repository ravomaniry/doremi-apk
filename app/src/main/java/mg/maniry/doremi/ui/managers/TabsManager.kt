...
class TabsManager(
    ...
    val explorerTab: View = View.inflate(mainContext, R.layout.file_explorer_tab, null)
    ...
    private fun initTabs() {
        tabSelectors = listOf(mainView.findViewById(R.id.editor_tab_selector),
                ...
                mainView.findViewById(R.id.explorer_tab_selector))

        uiViewModel.drawerTab.observe(mainContext as MainActivity, Observer(this::changeTabHandler))

        tabSelectors.forEachIndexed { i, sel -> sel.setOnClickListener { uiViewModel.changeTab(i) } }
    }
    ...
    private fun changeTabHandler(index: Int?) {
        ...
        contentContainer.apply { removeAllViews(); addView(activeTab); hideKeyboard() }
    }
}
